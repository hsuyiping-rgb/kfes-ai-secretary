/* ==========================================================================
   小光 AI 秘書 — 在地知識庫（家長/訪客問答資料）
   --------------------------------------------------------------------------
   本檔案定義全域變數 chatKnowledgeBase，供 app.js 的兩種問答模式共用：
     - 在地知識庫模式（無 API key）：getAIResponse() 以 userMessage.includes(key) 比對。
     - AI 智慧模式（有 Gemini key）：把這裡的主題當 RAG 背景餵給 Gemini。

   ⚠️ 載入順序：index.html 必須在 app.js「之前」載入本檔，app.js 才抓得到。

   ── 如何擴充 ──────────────────────────────────────────────────────────
   1. 新增主題：在 chatKnowledgeBase 物件內加一組 '關鍵字': `答案(Markdown)`。
      兩種模式會自動吃到，不需另外註冊。
   2. 新增同義詞/別名：在物件後面加 chatKnowledgeBase['別名'] = chatKnowledgeBase['主題'];
   3. 日期型 key（含數字＋「月/日/／」，如 6月24日 / 6/24）只有在使用者問到
      該日期時才會餵給 Gemini，避免 context 過長（判斷邏輯在 app.js）。
   ========================================================================== */
